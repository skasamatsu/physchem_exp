...
ある試験問題の正答率と標準偏差を計算するプログラムを作成せよ。
なお、標準偏差$\bar{s}$は平均値$\bar{x}$を求めたうえで、以下の式で求められる。
...
関数にインプットを与えることもできる。これを引数（ひきすう）と呼ぶ。
...
また、実行した結果として、値を返すこともできる。`return`という命令を使う。
...
これを応用すると、得点リストを受け取って平均点と合計点を計算する関数を以下のように書ける。
...
gokeiten, heikinten = gokei_heikin(tokuten) # 関数gokei_heikinを呼び出し、結果をgokeiten, heikintenに格納
print('合計= ', gokeiten)    # 合計点の表示
print('平均= ', heikinten)   # 平均点の表示
```
...
三角形の2辺の長さa、bとその挟む角θ(theta) が与えられた時、
残りの１辺の長さcを求め表示する関数を作ってみよう
（ヒント）余弦定理よりc<sup>2</sup> = a<sup>2</sup> + b<sup>2</sup> −2ab cos θ。a<sup>2</sup> の計算は`a**2`又は`a*a` と書く。√x の計算は``x**0.5``。
...
### 課題　プロットの確認
ｘ座標やy座標を変更すると、プロットされるグラフが変わることを確認せよ。
...
８種類の候補結晶それぞれに対してブラッグ回折が起こる角度$\theta$を計算する関数を作成せよ。
...
課題？で作ったそれぞれの物質の$\theta$リストをX線回折データに重ねて、物質の同定を行おう。
...
### 課題6-5  X線チャートに、物質名とミラー指数を鉛筆で書き込め（例：NaCl 200 など）
